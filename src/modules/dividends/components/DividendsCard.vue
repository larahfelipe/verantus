    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 mt-6">
      ...
      >
        <span class="text-[10px] font-bold uppercase tracking-wider text-neutral-400"
          >Dividend Yield</span
        >
        <div class="mt-2 text-xl font-black text-neutral-900 dark:text-white">
          {{ displayYield }}
        </div>
      ...
      >
        <span class="text-[10px] font-bold uppercase tracking-wider text-neutral-400"
          >Last / Ex-Date</span
        >
        <div class="mt-2 text-sm font-extrabold text-neutral-800 dark:text-neutral-200">
      ...
      <div class="flex items-start gap-3">
        ...
        <span class="text-neutral-700 dark:text-neutral-300 leading-relaxed">
          ...
          <span v-if="payoutUnderThreshold">
            A payout below 60% indicates that the dividend is well-covered by earnings, leaving
            substantial cash for reinvestment and buffer.
          </span>
          <span v-else>
            A payout exceeding 60% indicates that a large portion of earnings is returned to
            shareholders, which might limit high-velocity growth reinvestment.
          </span>
        </span>
      </div>
    </div>
...
const payoutUnderThreshold = computed(() => {
  const p = props.asset?.metrics.dividends.payoutRatio;
  return p !== null && p !== undefined && p < 0.6;
});
</script>
